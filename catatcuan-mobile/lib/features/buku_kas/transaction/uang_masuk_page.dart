...
class UangMasukPage extends StatefulWidget {
  const UangMasukPage({super.key});

  @override
  State<UangMasukPage> createState() => _UangMasukPageState();
}

class _UangMasukPageState extends State<UangMasukPage> {
  ...
  String _targetKas = 'laci'; // 'laci' (saldo_awal) or 'kas' (uang_kas)
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        await _supabase.from('BUKU_KAS').insert({
          ...
          'tipe': 'masuk',
          'sumber': 'manual_masuk', // Use specific enum value
          'amount': amount,
          'saldo_setelah': saldoSetelah,
          'keterangan': '[UANG MASUK - ${_targetKas.toUpperCase()}] ${_catatanController.text.trim()}',
        });
        ...
        if (mounted) {
          AppToast.showSuccess(context, 'Uang Masuk Berhasil Dicatat');
          context.pop(true);
        }
      } catch (e) {
        debugPrint('Error saving uang masuk: $e');
        if (mounted) {
          AppToast.showError(context, 'Gagal mencatat uang masuk: $e');
        }
      } finally {
  ...
  Widget _buildSourceChip(String value, String label, double bal) {
    ...
    return GestureDetector(
      onTap: () => setState(() => _targetKas = value),
      child: Container(
        height: 60,
        decoration: BoxDecoration(
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      style: TextStyle(
        ...
        color: isAmount ? AppTheme.primary : const Color(0xFF6B7280).withValues(alpha: 0.8),
        fontFamily: 'Poppins',
      ),
