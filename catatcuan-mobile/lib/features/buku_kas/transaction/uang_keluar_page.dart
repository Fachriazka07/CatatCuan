...
class _UangKeluarPageState extends State<UangKeluarPage> {
  ...
  String _sourceKas = 'laci'; // 'laci' (saldo_awal) or 'kas' (uang_kas)
  ...
  Future<void> _save() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (mounted) {
          AppToast.showSuccess(context, 'Uang Keluar Berhasil Dicatat');
          context.pop(true);
        }
      } catch (e) {
        debugPrint('Error saving uang keluar: $e');
        if (mounted) {
          AppToast.showError(context, 'Gagal mencatat uang keluar: $e');
        }
      } finally {
  ...
  Widget _buildSourceChip(String value, String label, double bal) {
    final bool isSel = _sourceKas == value;
    return GestureDetector(
      onTap: () => setState(() => _sourceKas = value),
      child: Container(
        height: 60,
        decoration: BoxDecoration(
          color: isSel
